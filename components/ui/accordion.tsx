...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitives.Header className="flex">
    <AccordionPrimitives.Trigger
      ref={ref}
      className={cn(
        "flex font-semibold items-center justify-between w-full group py-3 hover:underline",
        className
      )}
      {...props}
    >
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitives.Content
    className={cn(
      "transition-all data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down overflow-hidden",
      className
    )}
    ref={ref}
    {...props}
